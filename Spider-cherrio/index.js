...
var http = require('http'),
    ...
    path = require('path'),
    //node 后端 服务器  文件系统
    fs = require('fs'),    //前四个均是内置的库
    //从请求，响应，到分析，再到文件保存
    cheerio = require('cheerio');//开源库

//配置
var opt = {
    // URL 由子域名  路径(path) 路由  在哪个端口
...
//发出请求
const url = 'https://' + `${opt.hostname}${opt.path}?start=0`;   //start=0表示是第一页
//console.log(url);

https.get(url, function (res) {
    //二进制流，声明使用urt-8字符编码
    res.setEncoding('utf-8');
    //数据包到达  chunck这个二进制块  on data 事件
    var html = '';
    var movies = [];
    ...
    res.on('end', function () {
        //console.log(html)
        //cheerio将html字符串在命令行渲染显示出DOM的库
        var $ = cheerio.load(html);
        $('.item').each(function () {
            //获取图片的地址，图片不能直接爬下来，二进制流
